...
if [ "x$1" = "x" ]; then
  ...
  if [ "$MWE_ARCH" = "sol64" ] ; then
	LD_LIBRARY_PATH=.:/usr/lib/lwp:${MCRROOT}/runtime/${MWE_ARCH} ; 
  else
  ...
  if [ "$MWE_ARCH" = "maci" -o "$MWE_ARCH" = "maci64" ]; then
	...
	MCRJREVER=`cat ${MCRROOT}/sys/java/jre/${MWE_ARCH}/jre.cfg` ; 
	echo Found MCR Java JRE version: $MCRJREVER ; 
	MCRJRE=${MCRROOT}/sys/java/jre/${MWE_ARCH}/jre${MCRJREVER}/lib/i386 ;
